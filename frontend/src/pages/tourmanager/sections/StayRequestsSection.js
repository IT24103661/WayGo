...
export default function StayRequestsSection() {
  ...
  const { requests, loading, error, allocateStay, updateStayStatus, deleteStayAllocation } = useTourManagerStayRequests(statusFilter);
  ...
  const handleStatusUpdate = async (booking, nextStatus) => {
    ...
    const state = getFormForBooking(booking._id, booking);
    try {
      await updateStayStatus(booking._id, {
        stayStatus: nextStatus,
        stayManagerNotes: state.stayManagerNotes
      });
      setMessage(`Stay status updated to ${nextStatus}.`);
    } catch (err) {
      setMessage(err.message || 'Failed to update stay status.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold text-emerald-950">Stay Requests</h2>
        <p className="text-emerald-700/80">Allocate rooms and track stay readiness for tour bookings.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {['', ...STAY_STATUS_OPTIONS].map((status) => (
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-bold border transition-colors ${statusFilter === status ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-emerald-700 border-emerald-200 hover:bg-emerald-50'}`}
          >
            {status || 'All'}
          </button>
        ))}
      ...
      {message && (
        <p className="text-sm font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2">
          {message}
        </p>
      )}

      <div className="space-y-4">
        {loading && requests.length === 0 && <p className="text-emerald-700">Loading stay requests...</p>}
        {error && <p className="text-rose-600">{error}</p>}
        {!error && requests.length === 0 && !loading && (
          <p className="text-emerald-700/80">No stay requests found for this status.</p>
        )}

        {!error && requests.map((booking) => {
          ...
          return (
            <div key={booking._id} className="bg-white/90 rounded-3xl border border-emerald-200 p-5 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-bold text-emerald-950">{booking.tourPackage?.title || booking.packageOptions?.tourTitle || 'Tour Booking'}</p>
                  <p className="text-sm text-emerald-700/80">{booking.tourist?.name || 'Tourist'} • {booking.tourist?.phone || 'No phone'}</p>
                  <p className="text-xs text-emerald-700 mt-1">Current stay status: {booking.stayStatus || 'Awaiting Stay Allocation'}</p>
                ...
                <select
                  value={booking.stayStatus || 'Awaiting Stay Allocation'}
                  onChange={(event) => handleStatusUpdate(booking, event.target.value)}
                  disabled={loading}
                  className="px-3 py-2 rounded-xl border border-emerald-200 text-sm"
                >
                  {STAY_STATUS_OPTIONS.map((status) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 mt-4">
                <select
                  ...
                  className="px-3 py-2.5 rounded-xl border border-emerald-200 text-sm"
                >
                ...
                <button
                  ...
                  disabled={loading}
                  className="px-3 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 inline-flex items-center justify-center gap-2"
                >
              ...
              {Array.isArray(booking.stayAllocations) && booking.stayAllocations.length > 0 && (
                <div className="mt-4 border-t border-emerald-100 pt-3">
                  <p className="text-xs uppercase tracking-wide font-bold text-emerald-700 mb-2">Current Allocation</p>
                  <div className="space-y-2">
                    {booking.stayAllocations.map((allocation, index) => (
                      <div key={`${booking._id}-${index}`} className="text-sm text-emerald-900 bg-emerald-50/60 border border-emerald-100 rounded-xl px-3 py-2 flex items-center justify-between gap-2 flex-wrap">
                        <div className="flex items-center gap-2 flex-wrap">
                          <MdHotel className="text-emerald-700" />
