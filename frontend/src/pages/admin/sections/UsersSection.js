    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-4 border-b border-gray-100">
        <h2 className="text-base font-bold text-gray-800">
          All Users <span className="text-gray-400 font-normal text-sm">({filtered.length})</span>
        </h2>
        <div className="flex items-center gap-2">
          <input
            ...
            className="text-sm border border-gray-200 rounded-xl px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 w-52"
          />
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.map((u) => (
              <tr key={u.email} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-3">
                  <span className={`text-xs px-2.5 py-0.5 rounded-full font-semibold ${ROLE_COLOR[u.role] || 'bg-gray-100 text-gray-600'}`}>
                    {u.role}
                  </span>
                </td>
